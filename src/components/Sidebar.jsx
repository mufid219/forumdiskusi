  ...
  return (
    ...
    <header className="flex flex-col w-1/4 h-full justify-start items-start px-10 bg-white">
      ...
      <nav>
        ...
        <h1 className="text-xl flex flex-row cursor-pointer rounded-full p-3 hover:bg-slate-200 items-center"><BiLogIn className="mr-4" /><Link to="/login">Login</Link> </h1>
        <h1 className="text-xl flex flex-row cursor-pointer rounded-full p-3 hover:bg-slate-200 items-center "><BiLogOut className="mr-4" /><button type="button" onClick={signOut}>Logout</button> </h1>
        <h1 className="text-lg cursor-pointer py-3 px-5 bg-blue-700 text-white border-2 rounded-full ml-2 hover:bg-blue-600 focus:outline-none focus:bg-blue-600 flex justify-center"><Link to="/new">Buat diskusi</Link></h1>
      </nav>
    </header>
  );
}
